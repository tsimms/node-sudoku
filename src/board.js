...
class Board {
  constructor() {
    this.element = Array.from({ length: 9 }, () =>
  ...
  show = () =>
    `[ ${this.element.map(
      (rows, i) => `[ ${this.getRowVals(i + 1).toString()} ]`
    )} ]`;
  get = (col, row) => this.element[row - 1][col - 1];
  #getBox = ({ x, y }) =>
    this.element
      .map((row, yIndex) =>
        row.map((col, xIndex) => ({ ...col, col: xIndex + 1, row: yIndex + 1 }))
      )
      .slice((y - 1) * 3, y * 3)
      .map((row) => row.slice((x - 1) * 3, x * 3));
  #getRow = (row) => this.element.slice(row - 1, row)[0];
  #getCol = (col) => this.element.map((row) => row[col - 1]);

  getBoxCoord = (col, row) => ({
    x: Math.trunc((col + 2) / 3),
    y: Math.trunc((row + 2) / 3),
  });
  getBoxVals = ({ x, y }) =>
    this.#getBox({ x, y }).map((rows) => rows.map((col) => col.get()));
  getRowVals = (row) => this.#getRow(row).map((e) => e.get());
  getColVals = (col) => this.#getCol(col).map((e) => e.get());

  set = (col, row, number) => {
    this.get(col, row).set(number);
    return this;
  };
  load = (file) => {
  ...
  #getEmptyList = () => {
    const list = [];
    this.element.forEach((r, rIndex) =>
      r.forEach((c, cIndex) => {
        const col = cIndex + 1;
        const row = rIndex + 1;
        if (!this.get(col, row).isSet()) list.push({ col, row });
      })
    );
    return list;
  };
  ...
  setPossible = () => {
    ...
    empties.forEach(({ col, row }) => {
      ...
      const numbersInBox = this.getBoxVals(this.getBoxCoord(col, row))
        .flat()
        .filter(Number);
      const takenNumbers = [
        ...new Set([...numbersInRow, ...numbersInCol, ...numbersInBox]),
      ];
  ...
  #testOtherEmpties = (col, row) => {
    ...
    const testNotPossible = (col, row, test) => {
      ...
      const testCol = this.#getCol(col)
        ...
        .filter(
          (cell, index) =>
            JSON.stringify(this.getBoxCoord(col, index + 1)) !==
            JSON.stringify(baseBox)
        )
        .filter((cell) => !cell.isSet())
        .filter((cell) => cell.getPossible().includes(test))
        .map((cell) => {
          const { c, r } = cell;
          const boxList = this.#getBox(this.getBoxCoord(c, r))
            .flat()
            .filter((cell) => !cell.isSet())
            .filter((cell) => cell.getPossible().includes(test));
          const result = boxList.every((cell) => cell.col === col);
          return result;
        })
        .some((t) => t);
      if (testCol) {
        console.log(
          `Found at (${col}, ${row}), there's a column restriction on ${test}`
        );
      ...
      const testRow = this.#getRow(row)
        ...
        .filter(
          (cell, index) =>
            JSON.stringify(this.getBoxCoord(index + 1, row)) !==
            JSON.stringify(baseBox)
        )
        .filter((cell) => !cell.isSet())
        .filter((cell) => cell.getPossible().includes(test))
        .map((cell) => {
          ...
          const boxList = this.#getBox(this.getBoxCoord(c, r))
            ...
            .filter((cell) => !cell.isSet())
            .filter((cell) => cell.getPossible().includes(test));
          const result = boxList.every((cell) => cell.row === row);
          return result;
        })
        .some((t) => t);
      if (testRow) {
        console.log(
          `Found at (${col}, ${row}), there's a row restriction on ${test}`
        );
        this.get(col, row).cantBe(test);
      }
    };
    ...
    possibles.every((possible) => {
      ...
      if (
        testPossible(
          this.getColVals(col)
            ...
            .filter((cell) => !(cell.c === col && cell.r === row)),
          possible
        )
      ) {
        found = true;
      ...
      if (
        testPossible(
          this.getBoxVals(this.getBoxCoord(col, row))
            .flat()
            .map((cell, i) => ({
              val: cell,
  ...
  checkOtherEmpties = () => {
    ...
    for (let i = 0; i < empties.length; i++) {
      ...
      if (this.#testOtherEmpties(col, row)) return true;
    }
  };
